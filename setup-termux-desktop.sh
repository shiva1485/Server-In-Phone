...
${W}"
	confirmation_y_or_n "Do you want to Configuring File Manager Tools" fm_tools
	banner
...
function distro_questions() {
	...
	if [[ "$distro_add_answer" == "y" ]] && [[ "$zsh_answer" == "y" ]]; then
		banner
		confirmation_y_or_n "Do you want to Configuring Zsh also for Linux distro container" distro_zsh_answer
	else
...
function questions_install_type() {
	...
	echo "${G}Key Terms:${W}"
	echo "   • Generic: Pre-configured options that so far work best in most of the devices"
	echo "   • Recommended: Tested configurations with minimal known issues"
	echo "   • Hardware Acceleration: Uses your device's GPU for better graphics render"
	echo "   • Custom: Full control over all installation options"
	echo

	echo "${R}[${Y}!${R}]${R} Important Note: ${W}"
	echo -e "${B}
While 'Generic with Hardware Acceleration' works well on most modern devices, it may not be compatible with all hardware. 
...
Also most the installer's features are only available in custom option
${W}"
...
function install_required_packages() {
	...
	if [[ "$PACKAGE_MANAGER" == "pacman" ]]; then
		package_install_and_check "wget pv jq curl termux-am"
	else
		package_install_and_check "wget pv jq curl tar xz-utils gzip termux-am x11-repo tur-repo"
	fi
}
...
function install_desktop() {
	...
	if [[ "$desktop_answer" == "1" ]]; then
		...
		package_install_and_check "xfce4 xfce4-goodies xfce4-pulseaudio-plugin"
	elif [[ "$desktop_answer" == "2" ]]; then
